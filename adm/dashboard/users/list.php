...
if (CleanGET('acao') == 'excluir') {
    if (!emptyGET('id')) {
        ...
        $db1->table('users')
            ...
            ->where('idsite', $idsite)
            ->andNot()->where('permissao')->prepared(serialize(['nivel' => 'superadmin']))
            ->delete();
        $db1->close();

        redirect('/adm/dashboard/users/list.php');
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Lista de Usuários | MiConteudo</title>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <?php if (checkNivel() == 'superadmin') { ?>
                                        <a href="/adm/dashboard/users/edit.php" class="btn btn-success">Novo Usuário</a>
                                        <hr>
                                    <?php } ?>
                                    ...
                                    <table class="table table-hover text-nowrap">
                                        ...
                                        <tbody>
                                            ...
                                            while ($row = $db1->fetch()) {
                                                $db1->rows($row);
                                                $aPermissao = unserialize($db1->row('permissao'));
                                                $sNivel = $aPermissao['nivel'];
                                            ?>
                                                <tr>
                                                    <td>
                                                        <a href="/adm/dashboard/users/edit.php?id=<?php echo $db1->row('id'); ?>">
                                                            <?php echo $db1->row('usuario'); ?>
                                                        </a>
                                                    </td>
                                                    <td><?php echo $db1->row('nome'); ?></td>
                                                    <td><?php echo $db1->row('email'); ?></td>
                                                    <td><?php echo sprintf('Nível: %s', $sNivel); ?></td>
                                                    <td>
                                                        <?php if (checkNivel() == 'superadmin') { ?>
                                                            <a href="#" onclick="excluirRegistro('<?php echo $db1->row('id'); ?>');" class="btn btn-danger">Excluir</a>
                                                        <?php } ?>
                                                    </td>
                                                </tr>
                                            <?php }
    ...
    <script>
        function excluirRegistro(id) {
            if (confirm('Deseja realmente excluir esse registro?')) {
                window.location.assign('/adm/dashboard/users/list.php?acao=excluir&id=' + id);
            }
        }
    </script>
</body>
