...
if (requestPOST()) {
    if (emptyPOST('txtTitulo')) {
        windowAlert('Digite um título para sua postagem.');
    } else {
        $sAutor = CleanPOST('txtAutor');
        $sCategoria = CleanPOST('txtCategoria');
        ...
        $sImagens = serialize(['principal' => CleanPOST('txtImagens')]);
        ...
        if (empty($sID)) {
            ...
            $db1->table('posts')
                ->add('idsite', $idsite)
                ->add('idautor')->prepared($sAutor)
                ->add('idcategoria')->prepared($sCategoria)
...
if (!empty($sID)) {
    ...
    while ($row = $db1->fetch()) {
        $db1->rows($row);
        $txtAutor = $db1->row('idautor');
        $txtCategoria = $db1->row('idcategoria');
        ...
        $txtImagens = unserialize($db1->row('imagens'))['principal'];
...
<head>
    ...
    <title>Editar Postagem | MiConteudo</title>
                        ...
                        <div class="col-12">
                            <div class="card">
                                <div class="card-header">
                                    <a href="/adm/dashboard/post/list.php" class="btn btn-success">Lista de Postagens</a>
                                    <hr>
                                    <h3 class="card-title">Editar Postagem</h3>
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form id="quickForm" name="frmPost" method="post" action="?id=<?php echo $sID; ?>">
                                    <div class="card-body">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="txtAutor">Autores</label>
                                            <select id="txtAutor" name="txtAutor" class="form-control">
                                                ...
                                                while ($row = $db1->fetch()) {
                                                ?>
                                                    <option value="1" <?php echo ($txtAutor == $row['id']) ? 'selected="selected"' : ''; ?>><?php echo $row['nome']; ?></option>
                                                <?php }
                                                $db1->close();
                                                ?>
                                            </select>
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="txtCategoria">Categorias</label>
                                            <select id="txtCategoria" name="txtCategoria" class="form-control">
                                                ...
                                                while ($row = $db1->fetch()) {
                                                ?>
                                                    <option value="<?php echo $row['id']; ?>" <?php echo ($txtCategoria == $row['id']) ? 'selected="selected"' : ''; ?>><?php echo $row['titulo']; ?></option>
                                                <?php }
                                                $db1->close();
                                                ?>
                                            </select>
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="txtOutrasMetaTags1">Meta Tags (Dentro da tag head)</label>
                                            <textarea id="txtOutrasMetaTags1" name="txtOutrasMetaTags1" class="form-control"><?php echo $txtOutrasMetaTags1; ?></textarea>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <div class="form-group">
                                            <label for="txtOutrasMetaTags2">Meta Tags (Antes de fechar o body)</label>
