 ...
 * @version 2.2.8
 * 
 * Modificado por: Murilo Gomes Julio (Mestre da Info)
 * Site: https://www.mestredainfo.com.br
 */

namespace MiConteudo\template;
...
class template
{
	...
	protected function loadfile($varname, $filename)
	{
		if (!file_exists($filename)) {
			echo 'O arquivo ' . basename($filename) . ' não existe!';
			exit;
		}
	...
	protected function identifyBlocks(&$line, $varname, &$queued_blocks, &$blocks)
	{
		...
		if (1 == preg_match($reg, $line, $m)) {
			if (is_array($queued_blocks) && 0 == sizeof($queued_blocks)) {
				$parent = $varname;
			} else {
				$parent = (is_array($queued_blocks)) ? end($queued_blocks) : [];
			}
		...
		if (1 == preg_match($reg, $line) && is_array($queued_blocks)) {
	 ...
	 * @return array|string
	 */
	protected function substModifiers($value, $exp): array|string
	{
